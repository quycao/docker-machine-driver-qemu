...
public class JLANServer implements ServerListener {
  ...
  private ServerConfiguration m_srvConfig;
	...
	protected void start( String[] args) {
		...
		PrintStream err = createErrorStream();
    ...
    m_restart  = false;
		...
		try {
		  ...
		  checkPointError( err, CheckPointConfigLoading, ex);
			return;
		}
		...
		if ( CheckLocalIPAddress) {		
			...
			try {
			  ...
			  checkPointError( err, CheckPointCheckIPAddress, ex);
				return;
			}
		}
      ...
      while ( idx >= 0) {
		  ...
		  checkPointError( err, CheckPointServersStarted, ex);
		}
		finally {
  ...
  protected final NetworkServer createNFSPortMapper(ServerConfiguration config)
	...
	/**
	 * Create the error stream for logging
	 * 
	 * @return PrintStream
	 */
